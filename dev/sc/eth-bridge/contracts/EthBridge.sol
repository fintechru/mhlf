...
import './interfaces/EthBridgeInterface.sol';

contract EthBridge is FabricBridgeInterface {

    uint256 public i;
    string contractInformation = "EthBridgeV1";
    ...
    function setValueInEth(address methContract, uint256 _i) public {

        /// @dev 4byte сигнатуры метода, который вызываем
        bytes4 methodSelector = EthBridgeInterface(0).setValue.selector;
    ...
    function getValueInEth(address methContract) public returns (uint256) {

        /// @dev 4byte сигнатуры метода, который вызываем
        bytes4 methodSelector = EthBridgeInterface(0).getValue.selector;
